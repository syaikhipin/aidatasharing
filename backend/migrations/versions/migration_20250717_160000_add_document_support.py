...
def upgrade(connection):
    ...
    try:
        connection.execute(text("ALTER TABLE datasets ADD COLUMN document_type VARCHAR"))
        logger.info("Added column: document_type")
    except Exception as e:
        logger.warning(f"Failed to add document_type column: {e}")
    
    try:
        connection.execute(text("ALTER TABLE datasets ADD COLUMN page_count INTEGER"))
        logger.info("Added column: page_count")
    except Exception as e:
        logger.warning(f"Failed to add page_count column: {e}")
    
    try:
        connection.execute(text("ALTER TABLE datasets ADD COLUMN word_count INTEGER"))
        logger.info("Added column: word_count")
    except Exception as e:
        logger.warning(f"Failed to add word_count column: {e}")
    
    try:
        connection.execute(text("ALTER TABLE datasets ADD COLUMN extracted_text TEXT"))
        logger.info("Added column: extracted_text")
    except Exception as e:
        logger.warning(f"Failed to add extracted_text column: {e}")
    
    try:
        connection.execute(text("ALTER TABLE datasets ADD COLUMN text_extraction_method VARCHAR"))
...
def downgrade(connection):
    ...
    try:
        connection.execute(text("ALTER TABLE datasets DROP COLUMN document_type"))
        connection.execute(text("ALTER TABLE datasets DROP COLUMN page_count"))
        connection.execute(text("ALTER TABLE datasets DROP COLUMN word_count"))
        connection.execute(text("ALTER TABLE datasets DROP COLUMN extracted_text"))
        connection.execute(text("ALTER TABLE datasets DROP COLUMN text_extraction_method"))
