#!/usr/bin/env python3
"""
Comprehensive test of MindsDB Gemini integration
"""
...
def test_mindsdb_comprehensive():
    """Comprehensive test of MindsDB Gemini integration"""
    print("🧪 Comprehensive MindsDB Gemini test...")
    ...
    try:
        ...
        create_engine_sql = f"""
        CREATE ML_ENGINE IF NOT EXISTS test_google_gemini_engine
        FROM google_gemini
        USING
            api_key = '{api_key}';
        """
        ...
        create_model_sql = """
        CREATE MODEL IF NOT EXISTS mindsdb.test_gemini_simple
        PREDICT answer
        USING
            engine = 'test_google_gemini_engine',
        ...
        # Check model status
        print("5. Checking model status...")
        try:
            status_query = "DESCRIBE mindsdb.test_gemini_simple"
            status_result = connection.query(status_query)
            status_data = status_result.fetch()
        ...
        test_queries = [
            "SELECT answer FROM mindsdb.test_gemini_simple WHERE question = 'Hello'",
            "SELECT answer FROM mindsdb.test_gemini_simple WHERE question = 'What is 2+2?'",
            "SELECT answer FROM mindsdb.test_gemini_simple WHERE question = 'Say SUCCESS if you are working'"
        ]
        
        for i, test_query in enumerate(test_queries, 1):
            print(f"6.{i} Testing query: {test_query}")
            try:
                ...
                if result:
                    try:
                        if hasattr(result, 'fetch'):
                            ...
                            if not data.empty:
                                print(f"✅ SUCCESS! Model responded with data")
                                return True
                            else:
...
if __name__ == "__main__":
    ...
    success = test_mindsdb_comprehensive()
    
    print("\n" + "=" * 60)
    print("📊 TEST SUMMARY:")
    print(f"✅ Comprehensive MindsDB Test: {'PASSED' if success else 'FAILED'}")
    print("=" * 60)
