...
function UploadDatasetContent() {
  ...
  const handleSuccess = (datasetId: number) => {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-600">
            Add a new dataset to your organization for AI model training and analysis.
          </p>
        </div>
...
// Structured Data Uploader Component (existing functionality)
function StructuredDataUploader({ onSuccess, onError }: { onSuccess: (id: number) => void, onError: (error: any) => void }) {
  const router = useRouter();
  const [file, setFile] = useState<File | null>(null);
  const [isDragOver, setIsDragOver] = useState(false);
  const [isUploading, setIsUploading] = useState(false);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        onSuccess(1); // Replace with actual dataset ID from response
      }, 1000);
  ...
  return (
    <div className="space-y-8">
        ...
        >
          {!file ? (
            <>
              ...
              <p className="text-xs text-gray-500">CSV, JSON, Excel files up to 100MB</p>
              <input
                type="file"
      ...
      {/* Rest of the structured data upload UI */}
      {previewData && (
        <div className="bg-white shadow rounded-lg p-6">
